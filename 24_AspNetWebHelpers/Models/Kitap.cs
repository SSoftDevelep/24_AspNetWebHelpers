...
namespace _24_AspNetWebHelpers.Models
{
    public class Kitap
    {
        ...
        private static List<Kitap> GetFakeData(int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                kitaplar.Add(new Kitap()
                {
                    ...
                    YayinTarihi = FakeData.DateTimeData.GetDatetime(DateTime.Now.AddYears(-2), DateTime.Now.AddMonths(-1)),
                    Fiyat = FakeData.NumberData.GetNumber(8, 99)
                });
            }
            return kitaplar;
        }
